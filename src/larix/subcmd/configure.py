...
def command(namespace):
    ...
    build_path = (project_dir_path/'build'/namespace.target)
    build_path.mkdir(parents=True, exist_ok=True)

    project_yaml = yaml.load(project_file_path.open().read())
    project.configure(project_dir_path, build_path, project_yaml, namespace)

def add_sub_parser(subparsers):
    sub_parser = subparsers.add_parser('configure')
    sub_parser.set_defaults(sub_command='configure')
